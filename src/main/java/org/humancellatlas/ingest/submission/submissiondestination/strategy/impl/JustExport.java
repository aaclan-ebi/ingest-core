...
public class JustExport implements SubmissionStrategy {
  ...
  public void handleState(SubmissionEnvelope envelope, SubmissionState currentState) {
    if(currentState.equals(SubmissionState.SUBMITTED)) {
      ...
      throw new RuntimeException(String.format("Unexpected action %s for JustExport submission %s",
                                               currentState,
                                               envelope.getId()));
    }
  }
